...
export function validateGameValues(state: GameState): void {
  ...
  if (!Array.isArray(state.defeatedBosses)) {
...
export function getCurrentAct(totalCoffee: number): number {
...
export function hasPendingBoss(state: GameState): boolean {
...
export function buyUpgrade(state: GameState, upgradeKey: string): boolean {
...
export function calculateEffectiveCPS(state: GameState, now = Date.now()): number {
  ...
  if (state.thursdayModeUnlocked && state.cpsMultiplier !== 1.0) {
